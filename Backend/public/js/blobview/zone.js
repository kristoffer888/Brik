...
// Desc:
// Zone is the big ellipses that gets instantiated at canvas creation, they have a size,
// which is connected to the amount of people currently in the zone.
// They also have a color and name depending on their id.
...
function Zone(id) {
  ...
  this.init = function () {
    ...
    this.amountOfPeopleVisible = this.amountOfPeople;
  }

  //Movement and drawing of ellipse
  this.show = function() {
    //Invisible walls
    ...
    textSize(this.r * 0.2);
    textAlign(CENTER);
    text(this.amountOfPeopleVisible.toString(), this.pos.x, this.pos.y + this.r*0.25);
  }

  //Checks if it intersects the "other" zone
  ...
  //Called when it detects changes in amountOfPeople
  this.adjustSize = function(antal_Blobs, targetZone) {
      for(var i = 0; i < antal_Blobs; i++){
          //Create new blob with targetZone specified as their target.
          blobs.push(new Blob(this.pos.x, this.pos.y, 25, this.color, targetZone));
      }
  }

  //Called when a blob has entered or left a zone
